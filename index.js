...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (localStorage.getItem("sw_cleared_v4") !== "true") {
    ...
    localStorage.setItem("sw_cleared_v4", "true");
    setTimeout(() => {
...
function bindBillingFormInputs() {
  ...
  elements.billItemSelect.addEventListener("change", (e) => {
    ...
    if (prod) {
      ...
      elements.billItemGstRate.value = prod.gstRate !== undefined ? prod.gstRate.toString() : "5";
...
function calculateSummaryAndTable() {
  ...
  if (isLocal) {
    if (elements.sumCgstRow) elements.sumCgstRow.style.display = 'flex';
    if (elements.sumSgstRow) elements.sumSgstRow.style.display = 'flex';
    ...
    if (elements.sumIgstRow) elements.sumIgstRow.style.display = 'flex';
  }
...
function resetBillingForm() {
  ...
  elements.billPaidAmount.value = "0";
  if (elements.billPaidAmount) {
    elements.billPaidAmount.parentElement.style.display = "none";
  }
...
window.generateAndPrintThermal = function() {
  if (!currentInvoice.invoiceNo || !currentInvoice.buyer.name || currentInvoice.items.length === 0) {
    alert("Please fill invoice details and add items before printing!");
    return;
  }
  generateAndPrintInvoice();
  setTimeout(() => {
    printSavedInvoiceThermal(currentInvoice.invoiceNo);
  }, 300);
};
...
function populateA4PrintOverlay(invoice) {
  ...
  if (logoChoice === "ganesha") {
    ...
    if (mottoText) mottoText.textContent = "॥ श्री गणेशाय नमः ॥";
  ...
  if (document.getElementById("p-print-transport-mode")) {
    document.getElementById("p-print-transport-mode").textContent = invoice.transportMode || invoice.dispatchedThrough || "—";
  }
  ...
  invoice.items.forEach((item, index) => {
    ...
    tr.innerHTML = `
      ...
      <td style="text-align: left; font-weight: 700; color: #000;">${item.description}</td>
      <td style="text-align: center;">${item.hsn || "—"}</td>
      <td style="text-align: center;">${item.quantity} ${item.unit || 'Bucket'}</td>
  ...
  const qrCanvas = document.getElementById("p-print-qr-code");
  if (window.QRious && qrCanvas) {
    const amount = roundedGrandTotal;
    const upiUrl = `upi://pay?pa=${encodeURIComponent(activeUpi)}&pn=${encodeURIComponent(company.name || 'Aaryan Aqua Needs')}&am=${amount}&cu=INR`;
    new QRious({
      element: qrCanvas,
      value: upiUrl,
      size: 56
...
window.editSavedInvoice = function(id) {
  ...
  if (inv) {
    ...
    elements.billBundles.value = currentInvoice.bundles;
    elements.billSupplyPlace.value = currentInvoice.supplyPlace;
    elements.billSupplyStateCode.value = currentInvoice.supplyStateCode;
    ...
    elements.billPaidAmount.value = currentInvoice.paidAmount || currentInvoice.total;
...
function createPartyListCard(p) {
  ...
  card.innerHTML = `
    <div class="party-list-card-details">
      ...
      <span>GSTIN: ${p.gstin || "—"} ${p.phone ? '| Ph: ' + p.phone : ''}</span>
    </div>
...
window.exportSalesReportCSV = function() {
  ...
  csvContent += "Invoice No,Date,Document Type,Customer Name,GSTIN,Taxable Value,CGST,SGST,IGST,Total Amount,Payment Status,Payment Mode\n";
  ...
  link.setAttribute("download", `Aaryan_Aqua_GSTR1_Sales_Report_${new Date().toISOString().split('T')[0]}.csv`);
